...
const updateIssueEscalationById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await IssueEscalation.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const dbDoc = await IssueEscalation.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingIssueEscalationById",
      err,
    );
  }
};

module.exports = updateIssueEscalationById;

...
const getIssueEscalationStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(IssueEscalation.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            IssueEscalation.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await IssueEscalation.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingIssueEscalationStatsByQuery",
      err,
    );
  }
};

module.exports = getIssueEscalationStatsByQuery;

...
const getIssueEscalationAggById = async (issueEscalationId) => {
  try {
    let issueEscalationQuery;

    if (Array.isArray(issueEscalationId)) {
      issueEscalationQuery = IssueEscalation.find({
        _id: { $in: issueEscalationId },
        isActive: true,
      });
    } else {
      issueEscalationQuery = IssueEscalation.findOne({
    ...
    const issueEscalation = await issueEscalationQuery.exec();

    if (!issueEscalation) {
      return null;
    }
    const issueEscalationData =
      Array.isArray(issueEscalationId) && issueEscalationId.length > 0
        ? issueEscalation.map((item) => item.getData())
        : issueEscalation.getData();

    // should i add this here?
    await IssueEscalation.getCqrsJoins(issueEscalationData);

    return issueEscalationData;
  } catch (err) {
    console.log(err);
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingIssueEscalationAggById",
      err,
    );
  }
};
...
module.exports = getIssueEscalationAggById;

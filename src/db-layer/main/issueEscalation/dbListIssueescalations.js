...
class DbListIssueescalationsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListIssueescalations";
    this.emptyResult = true;
    this.objectName = "issueEscalations";
  ...
  async transposeResult() {
    for (const issueEscalation of this.dbData.items) {
  ...
  createQuery() {
    ...
    return IssueEscalation.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (IssueEscalation.getCqrsJoins) {
      await IssueEscalation.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await IssueEscalation.countDocuments(
      this.whereClause,
    );
  }
...
const dbListIssueescalations = (input) => {
  const dbGetListCommand = new DbListIssueescalationsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListIssueescalations;

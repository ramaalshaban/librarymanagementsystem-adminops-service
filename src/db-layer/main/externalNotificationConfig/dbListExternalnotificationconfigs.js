...
class DbListExternalnotificationconfigsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListExternalnotificationconfigs";
    this.emptyResult = true;
    this.objectName = "externalNotificationConfigs";
  ...
  async transposeResult() {
    for (const externalNotificationConfig of this.dbData.items) {
  ...
  createQuery() {
    ...
    return ExternalNotificationConfig.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (ExternalNotificationConfig.getCqrsJoins) {
      await ExternalNotificationConfig.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount =
      await ExternalNotificationConfig.countDocuments(this.whereClause);
  }
...
const dbListExternalnotificationconfigs = (input) => {
  const dbGetListCommand = new DbListExternalnotificationconfigsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListExternalnotificationconfigs;

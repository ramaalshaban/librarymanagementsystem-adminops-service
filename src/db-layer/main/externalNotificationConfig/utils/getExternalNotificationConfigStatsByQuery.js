...
const getExternalNotificationConfigStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(
          ExternalNotificationConfig.countDocuments(queryWithSoftDelete),
        );
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            ExternalNotificationConfig.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await ExternalNotificationConfig.countDocuments(
        queryWithSoftDelete,
      );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingExternalNotificationConfigStatsByQuery",
      err,
    );
  }
};

module.exports = getExternalNotificationConfigStatsByQuery;

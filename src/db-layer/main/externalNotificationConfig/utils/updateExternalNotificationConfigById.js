...
const updateExternalNotificationConfigById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ExternalNotificationConfig.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const dbDoc = await ExternalNotificationConfig.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingExternalNotificationConfigById",
      err,
    );
  }
};

module.exports = updateExternalNotificationConfigById;

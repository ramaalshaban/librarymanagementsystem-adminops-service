...
const getMongoAdminConfigStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(MongoAdminConfig.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            MongoAdminConfig.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await MongoAdminConfig.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingMongoAdminConfigStatsByQuery",
      err,
    );
  }
};

module.exports = getMongoAdminConfigStatsByQuery;

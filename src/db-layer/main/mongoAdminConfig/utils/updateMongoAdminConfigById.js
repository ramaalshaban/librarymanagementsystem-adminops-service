...
const updateMongoAdminConfigById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await MongoAdminConfig.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const dbDoc = await MongoAdminConfig.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingMongoAdminConfigById",
      err,
    );
  }
};

module.exports = updateMongoAdminConfigById;

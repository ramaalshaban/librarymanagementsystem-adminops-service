...
class DbListMongoadminconfigsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListMongoadminconfigs";
    this.emptyResult = true;
    this.objectName = "mongoAdminConfigs";
  ...
  async transposeResult() {
    for (const mongoAdminConfig of this.dbData.items) {
  ...
  createQuery() {
    ...
    return MongoAdminConfig.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (MongoAdminConfig.getCqrsJoins) {
      await MongoAdminConfig.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await MongoAdminConfig.countDocuments(
      this.whereClause,
    );
  }
...
const dbListMongoadminconfigs = (input) => {
  const dbGetListCommand = new DbListMongoadminconfigsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListMongoadminconfigs;

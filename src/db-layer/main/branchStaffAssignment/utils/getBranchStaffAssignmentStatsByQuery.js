...
const getBranchStaffAssignmentStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(
          BranchStaffAssignment.countDocuments(queryWithSoftDelete),
        );
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            BranchStaffAssignment.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await BranchStaffAssignment.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingBranchStaffAssignmentStatsByQuery",
      err,
    );
  }
};

module.exports = getBranchStaffAssignmentStatsByQuery;

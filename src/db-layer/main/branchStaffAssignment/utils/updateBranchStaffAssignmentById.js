...
const updateBranchStaffAssignmentById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await BranchStaffAssignment.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const dbDoc = await BranchStaffAssignment.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingBranchStaffAssignmentById",
      err,
    );
  }
};

module.exports = updateBranchStaffAssignmentById;

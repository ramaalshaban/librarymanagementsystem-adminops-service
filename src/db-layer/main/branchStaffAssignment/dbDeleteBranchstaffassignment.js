const {
  HttpServerError,
  BadRequestError,
  NotAuthenticatedError,
  ForbiddenError,
  NotFoundError,
} = require("common");
// do i need to add the referring part or does the mongodb use the things differently
// is there specific approch to handle the referential integrity or it done interrenly
const { BranchStaffAssignment } = require("models");
const { ObjectId } = require("mongoose").Types;

const {
  BranchStaffAssignmentQueryCacheInvalidator,
} = require("./query-cache-classes");

const { ElasticIndexer } = require("serviceCommon");

const { DBSoftDeleteMongooseCommand } = require("dbCommand");

class DbDeleteBranchstaffassignmentCommand extends DBSoftDeleteMongooseCommand {
  constructor(input) {
    const instanceMode = true;
    super(input, BranchStaffAssignment, instanceMode);
    this.commandName = "dbDeleteBranchstaffassignment";
    this.nullResult = false;
    this.objectName = "branchStaffAssignment";
    this.serviceLabel = "librarymanagementsystem-adminops-service";
    this.dbEvent =
      "librarymanagementsystem-adminops-service" +
      "-dbevent-" +
      "branchstaffassignment-deleted";
  }

  loadHookFunctions() {
    super.loadHookFunctions({});
  }

  initOwnership(input) {
    super.initOwnership(input);
  }

  async createQueryCacheInvalidator() {
    this.queryCacheInvalidator =
      new BranchStaffAssignmentQueryCacheInvalidator();
  }

  async indexDataToElastic() {
    const elasticIndexer = new ElasticIndexer(
      "branchStaffAssignment",
      this.session,
      this.requestId,
    );
    await elasticIndexer.deleteData(this.dbData.id);
  }

  // ask about this should i rename the whereClause to dataClause???

  async transposeResult() {
    // transpose dbData
  }
}

const dbDeleteBranchstaffassignment = async (input) => {
  input.id = input.branchStaffAssignmentId;
  const dbDeleteCommand = new DbDeleteBranchstaffassignmentCommand(input);
  return dbDeleteCommand.execute();
};

module.exports = dbDeleteBranchstaffassignment;

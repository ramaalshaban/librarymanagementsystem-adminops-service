...
class DbListBranchstaffassignmentsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListBranchstaffassignments";
    this.emptyResult = true;
    this.objectName = "branchStaffAssignments";
  ...
  async transposeResult() {
    for (const branchStaffAssignment of this.dbData.items) {
  ...
  createQuery() {
    ...
    return BranchStaffAssignment.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (BranchStaffAssignment.getCqrsJoins) {
      await BranchStaffAssignment.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await BranchStaffAssignment.countDocuments(
      this.whereClause,
    );
  }
...
const dbListBranchstaffassignments = (input) => {
  const dbGetListCommand = new DbListBranchstaffassignmentsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListBranchstaffassignments;

...
const getSystemBackupAuditStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(SystemBackupAudit.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            SystemBackupAudit.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await SystemBackupAudit.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingSystemBackupAuditStatsByQuery",
      err,
    );
  }
};

module.exports = getSystemBackupAuditStatsByQuery;

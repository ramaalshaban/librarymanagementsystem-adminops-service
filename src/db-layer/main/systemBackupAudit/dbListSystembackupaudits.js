...
class DbListSystembackupauditsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListSystembackupaudits";
    this.emptyResult = true;
    this.objectName = "systemBackupAudits";
  ...
  async transposeResult() {
    for (const systemBackupAudit of this.dbData.items) {
  ...
  createQuery() {
    ...
    return SystemBackupAudit.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (SystemBackupAudit.getCqrsJoins) {
      await SystemBackupAudit.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await SystemBackupAudit.countDocuments(
      this.whereClause,
    );
  }
...
const dbListSystembackupaudits = (input) => {
  const dbGetListCommand = new DbListSystembackupauditsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListSystembackupaudits;

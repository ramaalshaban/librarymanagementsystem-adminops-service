...
const getTregtStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(Tregt.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            Tregt.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await Tregt.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingTregtStatsByQuery",
      err,
    );
  }
};

module.exports = getTregtStatsByQuery;

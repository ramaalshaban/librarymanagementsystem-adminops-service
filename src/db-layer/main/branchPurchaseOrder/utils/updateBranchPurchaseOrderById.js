...
const updateBranchPurchaseOrderById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await BranchPurchaseOrder.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const dbDoc = await BranchPurchaseOrder.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingBranchPurchaseOrderById",
      err,
    );
  }
};

module.exports = updateBranchPurchaseOrderById;

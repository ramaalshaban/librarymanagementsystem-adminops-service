...
const getBranchPurchaseOrderStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(BranchPurchaseOrder.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            BranchPurchaseOrder.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await BranchPurchaseOrder.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingBranchPurchaseOrderStatsByQuery",
      err,
    );
  }
};

module.exports = getBranchPurchaseOrderStatsByQuery;

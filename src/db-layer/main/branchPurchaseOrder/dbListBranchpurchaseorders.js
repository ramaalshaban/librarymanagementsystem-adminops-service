...
class DbListBranchpurchaseordersCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListBranchpurchaseorders";
    this.emptyResult = true;
    this.objectName = "branchPurchaseOrders";
  ...
  async transposeResult() {
    for (const branchPurchaseOrder of this.dbData.items) {
  ...
  createQuery() {
    ...
    return BranchPurchaseOrder.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (BranchPurchaseOrder.getCqrsJoins) {
      await BranchPurchaseOrder.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await BranchPurchaseOrder.countDocuments(
      this.whereClause,
    );
  }
...
const dbListBranchpurchaseorders = (input) => {
  const dbGetListCommand = new DbListBranchpurchaseordersCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListBranchpurchaseorders;

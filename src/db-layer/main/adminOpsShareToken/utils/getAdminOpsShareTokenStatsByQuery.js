...
const getAdminOpsShareTokenStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(AdminOpsShareToken.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            AdminOpsShareToken.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await AdminOpsShareToken.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingAdminOpsShareTokenStatsByQuery",
      err,
    );
  }
};

module.exports = getAdminOpsShareTokenStatsByQuery;

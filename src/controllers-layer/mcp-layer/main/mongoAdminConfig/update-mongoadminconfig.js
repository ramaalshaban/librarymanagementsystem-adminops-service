...
class UpdateMongoAdminConfigMcpController extends AdminOpsMcpController {
  constructor(params) {
    super("updateMongoAdminConfig", "updatemongoadminconfig", params);
    this.dataName = "mongoAdminConfig";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateMongoAdminConfigManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Update status/settings of advanced MongoDB admin config or diagnostic record.",
    parameters: UpdateMongoAdminConfigMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const updateMongoAdminConfigMcpController =
        new UpdateMongoAdminConfigMcpController(mcpParams);
      try {
        const result =
          await updateMongoAdminConfigMcpController.processRequest();
        //return UpdateMongoAdminConfigMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",

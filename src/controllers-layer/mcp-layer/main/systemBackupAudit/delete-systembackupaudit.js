...
class DeleteSystemBackupAuditMcpController extends AdminOpsMcpController {
  constructor(params) {
    super("deleteSystemBackupAudit", "deletesystembackupaudit", params);
    this.dataName = "systemBackupAudit";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteSystemBackupAuditManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Archive, close, or delete a backup/restore operation audit record (soft delete).",
    parameters: DeleteSystemBackupAuditMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteSystemBackupAuditMcpController =
        new DeleteSystemBackupAuditMcpController(mcpParams);
      try {
        const result =
          await deleteSystemBackupAuditMcpController.processRequest();
        //return DeleteSystemBackupAuditMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",

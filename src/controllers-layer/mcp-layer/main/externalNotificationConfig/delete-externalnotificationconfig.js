...
class DeleteExternalNotificationConfigMcpController extends AdminOpsMcpController {
  constructor(params) {
    super(
  ...
  createApiManager() {
    return new DeleteExternalNotificationConfigManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Disable, archive, or delete notification/publishing config (soft delete).",
    parameters: DeleteExternalNotificationConfigMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteExternalNotificationConfigMcpController =
        new DeleteExternalNotificationConfigMcpController(mcpParams);
      try {
        const result =
          await deleteExternalNotificationConfigMcpController.processRequest();
        //return DeleteExternalNotificationConfigMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",

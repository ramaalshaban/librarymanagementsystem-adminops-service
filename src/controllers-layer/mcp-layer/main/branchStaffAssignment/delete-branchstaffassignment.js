...
class DeleteBranchStaffAssignmentMcpController extends AdminOpsMcpController {
  constructor(params) {
    super("deleteBranchStaffAssignment", "deletebranchstaffassignment", params);
    this.dataName = "branchStaffAssignment";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteBranchStaffAssignmentManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Delete or deactivate staff assignment from branch (soft delete).",
    parameters: DeleteBranchStaffAssignmentMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteBranchStaffAssignmentMcpController =
        new DeleteBranchStaffAssignmentMcpController(mcpParams);
      try {
        const result =
          await deleteBranchStaffAssignmentMcpController.processRequest();
        //return DeleteBranchStaffAssignmentMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",

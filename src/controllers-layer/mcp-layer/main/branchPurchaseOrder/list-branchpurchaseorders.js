...
class ListBranchPurchaseOrdersMcpController extends AdminOpsMcpController {
  constructor(params) {
    super("listBranchPurchaseOrders", "listbranchpurchaseorders", params);
    this.dataName = "branchPurchaseOrders";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListBranchPurchaseOrdersManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "List branch procurement/acquisition purchase orders (filter by status/branch/approval etc).",
    parameters: ListBranchPurchaseOrdersMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listBranchPurchaseOrdersMcpController =
        new ListBranchPurchaseOrdersMcpController(mcpParams);
      try {
        const result =
          await listBranchPurchaseOrdersMcpController.processRequest();
        //return ListBranchPurchaseOrdersMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",

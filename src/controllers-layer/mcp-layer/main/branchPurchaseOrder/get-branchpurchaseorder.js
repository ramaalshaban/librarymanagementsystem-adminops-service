...
class GetBranchPurchaseOrderMcpController extends AdminOpsMcpController {
  constructor(params) {
    super("getBranchPurchaseOrder", "getbranchpurchaseorder", params);
    this.dataName = "branchPurchaseOrder";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetBranchPurchaseOrderManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Fetch a library branch acquisition (purchase order) record by ID.",
    parameters: GetBranchPurchaseOrderMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getBranchPurchaseOrderMcpController =
        new GetBranchPurchaseOrderMcpController(mcpParams);
      try {
        const result =
          await getBranchPurchaseOrderMcpController.processRequest();
        //return GetBranchPurchaseOrderMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",

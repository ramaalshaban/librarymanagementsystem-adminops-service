...
class ListIssueEscalationsMcpController extends AdminOpsMcpController {
  constructor(params) {
    super("listIssueEscalations", "listissueescalations", params);
    this.dataName = "issueEscalations";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListIssueEscalationsManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "List escalated issues (filter by branch, user, status, type, assignment).",
    parameters: ListIssueEscalationsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listIssueEscalationsMcpController =
        new ListIssueEscalationsMcpController(mcpParams);
      try {
        const result = await listIssueEscalationsMcpController.processRequest();
        //return ListIssueEscalationsMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
